...
export class UnbufferredChannel<T> implements Channel<T>, PutChannel<T>, AsyncIterableIterator<T> {
    ...
    next(): Promise<{ value: T, done: false } | { value: T | undefined, done: true }> {
        if (this._closed) {
            return Promise.resolve({ value: this.defaultValue, done: true });
        }
    ...
    async close(defaultValue?: T): Promise<void> {
        if (this._closed) {
